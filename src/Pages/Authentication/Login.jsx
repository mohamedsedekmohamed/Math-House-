...
const Login = () => {
    ...
    const getQueryParams = () => {
        ...
        if (userParam) {
            try {
                ...
                console.error('Failed to parse user query parameter:', error);
                return null;
            }
        }
        return null;
    ...
    // Handle auto-login from query parameters
    useEffect(() => {
        // Only attempt auto-login if it hasn't been attempted yet
    ...
    // Handle API response for manual login
    useEffect(() => {
        if (response && response.data && response.status === 200 && !loadingPost) {
            auth.login(response.data?.user);
            navigate('/courses', { replace: true });
        }
    }, [response, loadingPost, auth, navigate]);
